...
class ElfFile:
    ...
    def add_section(
        self, name, sh_type, sh_flags, sh_link, sh_info, sh_addralign, sh_entsize, data
    ):
        shstr = self.sections[self.elf_header.e_shstrndx]
    ...
    for raw_line in f:
        ...
        def add_sized(size):
            if cur_section in [".text", ".late_rodata"]:
                assert size % 4 == 0, (
                    "size must be a multiple of 4 on line: " + raw_line
                )
            assert size >= 0
            fn_section_sizes[cur_section] += size
            if cur_section == ".text":
                assert (
                    first_fn_name is not None
            ...
            if line.startswith('");'):
                ...
                if fn_section_sizes[".late_rodata"] > 0:
                    ...
                    for i in range(0, size * 3, 3):
                        ...
                        late_rodata_fn_output.append(
                            "*(volatile float*)0 = {}f;".format(fval)
                        )
                        late_rodata_fn_output.append("")
                        late_rodata_fn_output.append("")
                temp_fn_name = None
                if fn_section_sizes[".text"] > 0 or late_rodata_fn_output:
                    ...
                    for (line, count) in fn_ins_inds:
                        for _ in range(count):
                            if (
                                ...
                                and instr_count - tot_emitted > min_instr_count
                                ...
                                fn_emitted = 0
                                fn_skipped = 0
                                output_lines[line] += " }} void {}(void) {{ ".format(
                                    make_name("large_func")
                                )
                    ...
                    if rodata_stack:
                        ...
                        print(
                            "late rodata to text ratio is too high: {} / {} must be <= 1/3".format(
                                size, available
                            ),
                            file=sys.stderr,
                        )
                ...
                if (
                    ...
                    and first_fn_name is None
                    and cur_section == ".text"
                ):
                    first_fn_name = line.split()[1]
                if not line:
                    pass  # empty line
                elif line.startswith("glabel ") or (
...
def fixup_objfile(objfile_name, functions, asm_prelude, assembler):
    ...
    o_file = tempfile.NamedTemporaryFile(
        prefix="asm-processor", suffix=".o", delete=False
    )
    o_name = o_file.name
    o_file.close()
    s_file = tempfile.NamedTemporaryFile(
        prefix="asm-processor", suffix=".s", delete=False
    )
    s_name = s_file.name
    try:
        ...
        # conflicts.
        new_local_syms = [
            s for s in objfile.symtab.local_symbols() if not is_temp_name(s.name)
        ]
        new_global_syms = [
            s for s in objfile.symtab.global_symbols() if not is_temp_name(s.name)
        ]
        for i, s in enumerate(asm_objfile.symtab.symbol_entries):
            ...
            if s.st_shndx != SHN_UNDEF:
                ...
                ), "Generated assembly .o must only have symbols for .text, .data, .rodata and UNDEF, but found {}".format(
                    section_name
                )
                ...
                if s.name in first_fn_names:
                    s.type = STT_FUNC
            if (
                objfile.sections[s.st_shndx].name == ".rodata"
                and s.st_value in moved_late_rodata
            ):
        ...
        for sectype in SECTIONS:
            ...
            if target is not None:
                ...
                for reltab in target.relocated_by:
                    ...
                    for rel in reltab.relocations:
                        ...
                        rel.sym_index = objfile.symtab.symbol_entries[
                            rel.sym_index
                        ].new_index
                        nrels.append(rel)
            ...
            for reltab in source.relocated_by:
                for rel in reltab.relocations:
                    rel.sym_index = asm_objfile.symtab.symbol_entries[
                        rel.sym_index
                    ].new_index
...
def main():
    ...
    parser.add_argument(
        "--post-process", dest="objfile", help="path to .o file to post-process"
    )
    parser.add_argument(
        "--assembler",
        dest="assembler",
